...
public class GraphicalDeck {
     ...
     */
    public GraphicalDeck(boolean includeJokers) {
        if (includeJokers)
            deck = new GraphicalCard[54];
        else
            deck = new GraphicalCard[52];
        int cardCt = 0; // How many cards have been created so far.
        for ( int suit = 0; suit <= 3; suit++ ) {
            for ( int value = 2; value <= 14; value++ ) {
                deck[cardCt] = new GraphicalCard(value,suit);
                cardCt++;
            }
        }
        if (includeJokers) {
            deck[52] = new GraphicalCard(1,GraphicalCard.JOKER);
            deck[53] = new GraphicalCard(2,GraphicalCard.JOKER);
        }
        cardsUsed = 0;
    }
